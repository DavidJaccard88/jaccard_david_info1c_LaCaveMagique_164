    ...
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_films_afficher_data = """SELECT id_bouteille, nom_bouteille, prix_bouteille,
                                                       GROUP_CONCAT(cepage_bouteille) as CepagesBouteilles FROM t_avoir_cepage
                                                       INNER JOIN t_bouteille ON t_bouteille.id_bouteille = t_avoir_cepage.fk_bouteille
                                                       INNER JOIN t_cepage_bouteille ON t_cepage_bouteille.id_cepage_bouteille = t_avoir_cepage.fk_cepage
                                                       GROUP BY id_bouteille"""
                ...
                if id_film_sel == 0:
                    ...
                    strsql_genres_films_afficher_data += """ HAVING id_bouteille= %(value_id_bouteille_selected)s"""
                ...
                if not data_genres_films_afficher and id_film_sel == 0:
                    ...
                    flash(f"La bouteille {id_film_sel} demandé n'existe pas !!", "warning")
                else:
                    flash(f"Données Bouteille affichés !!", "success")
...
def edit_genre_film_selected():
    if request.method == "GET":
        try:
            with DBconnection() as mc_afficher:
                strsql_genres_afficher = """SELECT id_cepage_bouteille, t_cepage_bouteille FROM  ORDER BY id_cepage_bouteille ASC"""
            ...
            lst_data_genres_films_non_attribues = [item['id_cepage_bouteille'] for item in data_genres_films_non_attribues]
            ...
            lst_data_genres_films_old_attribues = [item['id_cepage_bouteille'] for item in data_genres_films_attribues]
...
def update_genre_film_selected():
    if request.method == "POST":
        try:
            ...
            strsql_insert_genre_film = """INSERT INTO t_avoir_cepage (id_avoir_cepage, fk_cepage, fk_bouteille)
            ...
            strsql_delete_genre_film = """DELETE FROM t_avoir_cepage WHERE fk_cepage = %(value_fk_cepage)s AND fk_bouteille = %(value_fk_bouteille)s"""
...
def genres_films_afficher_data(valeur_id_film_selected_dict):
    ...
    try:
        ...
        strsql_film_selected = """SELECT id_bouteille, nom_bouteille,prix_bouteille, GROUP_CONCAT(id_cepage_bouteille) as CepagesBouteilles FROM t_avoir_cepage
                                        INNER JOIN t_bouteille ON t_bouteille.id_bouteille = t_avoir_cepage.fk_bouteille
                                        INNER JOIN t_cepage_bouteille ON t_cepage_bouteille.id_cepage_bouteille = t_avoir_cepage.fk_cepage
                                        WHERE id_bouteille = %(value_id_film_selected)s"""

        strsql_genres_films_non_attribues = """SELECT id_cepage_bouteille, cepage_bouteille FROM t_cepage_bouteille WHERE id_cepage_bouteille not in(SELECT id_cepage_bouteille as idGenresFilms FROM t_avoir_cepage
                                                    INNER JOIN t_bouteille ON t_bouteille.id_bouteille = t_avoir_cepage.fk_bouteille
                                                    INNER JOIN t_cepage_bouteille ON t_cepage_bouteille.id_cepage_bouteille = t_avoir_cepage.fk_cepage
                                                    WHERE id_bouteille = %(value_id_film_selected)s)"""

        strsql_genres_films_attribues = """SELECT id_bouteille, id_cepage_bouteille, cepage_bouteille FROM t_avoir_cepage
                                            INNER JOIN t_bouteille ON t_bouteille.id_bouteille = t_avoir_cepage.fk_bouteille
                                            INNER JOIN t_cepage_bouteille ON t_cepage_bouteille.id_cepage_bouteille = t_avoir_cepage.fk_cepage
                                            WHERE id_bouteille = %(value_id_film_selected)s"""
